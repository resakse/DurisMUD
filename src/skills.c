...
//  SPELL_ADD(CLASS_DRUID, 4);
  ...
  SPELL_CREATE("call archon", SPELL_ANIMATE_DEAD, PULSE_SPELLCAST * 3,
...
//  SPELL_ADD(CLASS_BARD, 3);
  ...
  SPEC_SPELL_ADD(CLASS_ANTIPALADIN, 12, SPEC_DARKKNIGHT);
  ...
  SPELL_CREATE("creeping doom", SPELL_CDOOM, PULSE_SPELLCAST * 3,
                TAR_AREA | TAR_OFFAREA | TAR_AGGRO, spell_cdoom);
...
// Ethermancer
 ...
 // SPEC_SPELL_ADD(CLASS_ETHERMANCER, 10, SPEC_WINDTALKER);
  ...
  SPELL_CREATE("holy blade", SPELL_DOOM_BLADE, PULSE_SPELLCAST * 8,
                TAR_IGNORE | TAR_NOCOMBAT, spell_doom_blade);
...
// Creating this bogus skill so only the person being throat crushed receives the wear off message.
  ...
  SPEC_SKILL_ADD(CLASS_WARRIOR, 0, 0, SPEC_SWASHBUCKLER);
  SPEC_SKILL_ADD(CLASS_WARRIOR, 31, 100, SPEC_GUARDIAN);
  SPEC_SKILL_ADD(CLASS_ANTIPALADIN, 20, 90, SPEC_DEMONIC);
  SPEC_SKILL_ADD(CLASS_PALADIN, 20, 90, SPEC_CAVALIER);
  ...
  SKILL_ADD(CLASS_PALADIN, 56, 60);
  SKILL_ADD(CLASS_ANTIPALADIN, 56, 70);
